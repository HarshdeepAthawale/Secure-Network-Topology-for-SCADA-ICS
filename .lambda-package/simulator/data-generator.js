...
class DataGenerator {
    ...
    async start() {
        logger_1.logger.info('Starting SCADA data generator', { broker: this.brokerUrl, topic: this.topic });
        ...
        await new Promise((resolve, reject) => {
            ...
            this.client.on('connect', () => {
                ...
                logger_1.logger.info('Data generator connected to MQTT broker');
                resolve();
            });
    ...
    async stop() {
        ...
        logger_1.logger.info('Data generator stopped');
    }
    async generateAndPublish() {
        ...
        try {
            ...
            logger_1.logger.info(`Tick ${this.tickCount}: published telemetry batch`, {
            ...
            logger_1.logger.error('Error generating telemetry', { error: err.message });
        }
    }
    // --------------------------------------------------------------------------
    // SNMP Telemetry Generation
    // --------------------------------------------------------------------------
    generateSNMPBatch(now) {
    ...
    publish(message) {
        if (!this.client || !this.client.connected) {
            logger_1.logger.warn('MQTT client not connected, skipping publish');
            return;
        }
...
if (require.main === module) {
    ...
    const shutdown = async () => {
        logger_1.logger.info('Shutting down data generator...');
    ...
    generator.start().catch(err => {
        logger_1.logger.error('Failed to start data generator', { error: err.message });
        process.exit(1);
    });
}
