...
const getIoTDataClient = (): AWS.IotData => {
...
// Simulated device definitions (same as data-generator.ts)
interface SimDevice {
  name: string;
  ip: string;
  mac: string;
 ...
 */
export async function handler(event: ScheduledEvent): Promise<GenerationResult> {
  const requestId = generateUUID();
  logger.setContext({ requestId, function: 'generator' });
  logger.info('Generating SCADA telemetry data');
  ...
  if (!IOT_ENDPOINT) {
    ...
    logger.error('IoT endpoint not configured');
    return result;
  }
  ...
  try {
    ...
    for (const telemetry of snmpBatch) {
      try {
        ...
        logger.error('Failed to publish SNMP telemetry', { 
          error: errorMsg,
    ...
    try {
      ...
      const errorMsg = error instanceof Error ? error.message : String(error);
      result.errors.push(errorMsg);
      logger.error('Failed to publish ARP telemetry', { 
        error: errorMsg,
        stack: error instanceof Error ? error.stack : undefined
      });
    }
    ...
    try {
      ...
      const errorMsg = error instanceof Error ? error.message : String(error);
      result.errors.push(errorMsg);
      logger.error('Failed to publish NetFlow telemetry', { 
        error: errorMsg,
        stack: error instanceof Error ? error.stack : undefined
      });
    }
    ...
    if (invocationCount === 0) {
      ...
      try {
        ...
        const errorMsg = error instanceof Error ? error.message : String(error);
        result.errors.push(errorMsg);
        logger.error('Failed to publish Syslog telemetry', { 
          error: errorMsg,
          stack: error instanceof Error ? error.stack : undefined
        });
      }
    ...
    logger.exception(error as Error, 'Data generation failed');
  }
 ...
 */
async function publishToIoT(message: object): Promise<void> {
  try {
    const payload = JSON.stringify(message);
    const iotDataClient = getIoTDataClient();
    
    const params: AWS.IotData.PublishRequest = {
    ...
    const errorDetails = error as AWS.AWSError;
 ...
 */
function generateSNMPBatch(now: Date): object[] {
 ...
 * Generate ARP telemetry
 */
function generateARPTelemetry(now: Date): object {
 ...
 * Generate NetFlow telemetry
 */
function generateNetFlowTelemetry(now: Date): object {
 ...
 */
function generateSyslogTelemetry(now: Date): object {
  const templates = [
