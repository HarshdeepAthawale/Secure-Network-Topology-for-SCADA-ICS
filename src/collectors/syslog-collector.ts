...
import { SyslogError } from '../utils/error-handler';
...
interface ParsedSyslogMessage extends SyslogMessage {
  sourceIP?: string;
}
...
export class SyslogCollector extends BaseCollector {
  ...
  constructor(collectorConfig?: Partial<CollectorConfig>) {
    ...
    super('SyslogCollector', TelemetrySource.SYSLOG, mergedConfig);

    this.port = config.syslog.port;
    this.protocol = config.syslog.protocol;
  }
  ...
  protected async cleanup(): Promise<void> {
    logger.info('Cleaning up Syslog collector');
  ...
  private async startUDPServer(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      this.udpServer.on('error', (error) => {
        logger.error('Syslog UDP server error', { error: error.message });
        reject(new SyslogError(`UDP server error: ${error.message}`));
      });
      ...
      this.udpServer.on('listening', () => {
        ...
        logger.info('Syslog UDP server listening', {
    ...
    return new Promise((resolve, reject) => {
      this.tcpServer = net.createServer((socket) => {
        ...
        socket.on('error', (error) => {
          logger.error('Syslog TCP connection error', {
            error: error.message,
            source: sourceIP,
          });
        });
      ...
      this.tcpServer.on('error', (error) => {
        logger.error('Syslog TCP server error', { error: error.message });
        reject(new SyslogError(`TCP server error: ${error.message}`));
      });

      this.tcpServer.listen(this.port, () => {
        logger.info('Syslog TCP server listening', { port: this.port });
        resolve();
      });
    });
  }
  ...
  protected async collect(_target: CollectorTarget): Promise<TelemetryData[]> {
    ...
    if (this.messageBuffer.length > 0) {
      ...
      logger.debug('Syslog collection completed', {
   ...
   */
  private handleMessage(rawMessage: string, sourceIP: string): void {
    try {
      ...
      logger.debug('Failed to parse syslog message', {
   ...
   */
  private parseMessage(raw: string): ParsedSyslogMessage | null {
   ...
   */
  private parseRFC5424(raw: string): ParsedSyslogMessage | null {
   ...
   */
  private parseRFC3164(raw: string): ParsedSyslogMessage | null {
   ...
   */
  private parseStructuredData(sd: string): Record<string, string> | undefined {
  ...
  private addToBuffer(message: ParsedSyslogMessage): void {
    this.messageBuffer.push(message);

    if (this.messageBuffer.length > this.maxBufferSize) {
      this.messageBuffer = this.messageBuffer.slice(-this.maxBufferSize);
      logger.warn('Syslog buffer overflow - oldest messages dropped');
    }
  }
   ...
   */
  private groupBySeverity(messages: ParsedSyslogMessage[]): Record<string, number> {
    const result: Record<string, number> = {};

    for (const msg of messages) {
      const severity = SYSLOG_SEVERITIES[msg.severity] || 'unknown';
   ...
   */
  private isSecurityRelevant(message: ParsedSyslogMessage): boolean {
    // High severity is always relevant
    if (message.severity <= 3) return true;
   ...
   */
  private getTopHosts(messages: ParsedSyslogMessage[], limit = 10): Array<{ host: string; count: number }> {
  ...
  getFacilityName(facility: number): string {
    return SYSLOG_FACILITIES[facility] || 'unknown';
  }
  ...
  getSeverityName(severity: number): string {
    return SYSLOG_SEVERITIES[severity] || 'unknown';
  }
...
export function createSyslogCollector(config?: Partial<CollectorConfig>): SyslogCollector {
