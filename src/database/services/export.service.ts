...
import { TopologySnapshot, Device, Connection, Alert, AlertSeverity } from '../../utils/types';
...
export class ExportService {
     ...
     */
    async exportTopology(options: ExportOptions): Promise<string> {
     ...
     */
    async generateComplianceReport(framework: 'NIST' | 'IEC62443' | 'NERC-CIP' | 'custom'): Promise<ComplianceReport> {
        ...
        const unknownDevices = devices.filter((d: Device) => d.type === 'unknown');
        if (unknownDevices.length > 0) {
            findings.push({
                ...
                severity: AlertSeverity.MEDIUM,
                affectedDevices: unknownDevices.map((d: Device) => d.id),
                status: 'partial',
        ...
        if (offlineDevices.length > 0) {
            findings.push({
                ...
                severity: offlineDevices.length > 5 ? AlertSeverity.HIGH : AlertSeverity.LOW,
                affectedDevices: offlineDevices.map((d: Device) => d.id),
                status: 'non-compliant',
        ...
        const summary = {
            ...
            criticalFindings: findings.filter(f => f.severity === AlertSeverity.CRITICAL).length,
            highFindings: findings.filter(f => f.severity === AlertSeverity.HIGH).length,
            mediumFindings: findings.filter(f => f.severity === AlertSeverity.MEDIUM).length,
            lowFindings: findings.filter(f => f.severity === AlertSeverity.LOW).length,
            complianceScore,
        };

        logger.info('Compliance report generated', { framework, score: complianceScore, findings: findings.length });
     ...
     */
    async generateNetworkDiagram(): Promise<NetworkDiagramData> {
        ...
        const zoneConfigs: Record<string, { name: string; level: number; color: string }> = {
        ...
        // Build zones
        const zoneDevices = new Map<string, string[]>();
     ...
     */
    async exportDeviceInventory(format: ExportFormat = 'csv'): Promise<string> {
     ...
     */
    async exportAlertHistory(days = 30, format: ExportFormat = 'csv'): Promise<string> {
     ...
     */
    private async gatherExportData(options: ExportOptions): Promise<{
        devices?: Device[];
     ...
     */
    private toCSV(data: { devices?: Device[] }): string {
     ...
     * Convert to XML
     */
    private toXML(data: { devices?: Device[]; connections?: Connection[] }): string {
     ...
     */
    private toHTML(data: { devices?: Device[]; connections?: Connection[]; metadata?: Record<string, unknown> }): string {
        return `
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
  ...
  <table>
     ...
     */
    private getDeviceShape(type: string): string {
